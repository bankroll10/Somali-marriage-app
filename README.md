...
docs/THREAT.md          STRIDE threat model: every public endpoint and sensitive flow, eight attacker personas, threats ranked likelihood × impact × ease; P0 built (the vouch read capped, the safety channel un-buryable, no-store on personal reads, no code on disk from the offline shell, no erase button offered for a dropped signal), P1/P2 and accepted-by-design recorded
docs/SECURITY.md        OWASP audit, the implementation read rather than trusted: nineteen focus areas with verdicts and evidence; eleven findings written as attacks, run against the old code, then fixed (forget me erasing a victim's reports, a restore link taking over a phone, crashing bodies, an unmetered token oracle, her eleven rewritable by him, no framing protection, six-character codes giving one script ~14% of maps a year → eight); what was verified sound, what is accepted, P2
docs/PRIVACY.md         Privacy by Design: every field and event, each asked why / when / where / who reads it / how long / how deleted / could we do without it; seven collections removed or minimized (a kept map's ms timestamps and last-seen time, the advice line, the guide's name and exact age, a local activity diary, the form's copy of every contact), six retention gaps closed (orphan vouches, expired sheets, lapsed step counts, contact exports, backup artifact, guide threads); Trust shorter, not longer; encryption next, after minimizing
docs/ABUSE.md           Fourteen abuse cases (harassment, impersonation, stalking, coercion, catfishing, the married man, blackmail, screenshots, ban evasion, family, false reports, spam, romance scams, social engineering) walked through the product, each sorted prevent / detect / respond / cannot yet; built: a report outlives the sheet, change my code, the help line per country, the read's money question, forget me no longer withdraws reports, a link that wiped a phone made inert; what is refused because it is surveillance
docs/GUIDE-EVAL.md      The Guide, measured: 54 cases in 17 categories (ghosting to self-harm to prompt injection), nine dimensions scored by rules and by a judge, hard gates on safety, integrity, invention and religious deference; the offline voice graded on every PR against a baseline that may only rise, the live model on demand (`npm run eval:guide`); what the first run found (no crisis handling anywhere) and what it built
