...
| **3 · No code at all** | `ops` | How the service did each day: server errors by route, caps that refused by kind, the guide's calls and tokens, crashes phones reported. A closed list of signals, a day each, kept thirty-five days (`docs/OPS.md`) | Founder, through `/health` | Nothing — no id, no city, no time; it describes the service, never a person |
