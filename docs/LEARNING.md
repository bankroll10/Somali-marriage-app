...
| **User feedback** | Follow-up outcome *asked* (`through`; *differently* and *not yet* stay on the phone — their share per topic is read as `eleven.open` less `through`); `ending.used`; *(designed)* one tap per instrument, about the product never a person | Which instruments are decoration | Ratings, NPS, surveys — feedback that becomes its own loop |
