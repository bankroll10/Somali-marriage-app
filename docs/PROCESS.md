...
| Opening a pool | `docs/LIQUIDITY.md`'s checklist, every line — twenty active preparing men and at least as many women, everyone with someone eligible and active (read raw, not floored), seven in ten answering within a fortnight, three of the first twenty introductions talking, arrivals keeping pace — or it does not open. Anyone stranded names the side to find, never a band to widen | `docs/ATOMIC.md` §6, `docs/LIQUIDITY.md`, logged as A7 |
...
| The ending's door share | Ten endings and `sidesByVia.man.married.arrived` still `null` → the ask is rewritten once; null after ten more → dropped, the eleven alone | `docs/FLYWHEEL.md`, logged as A8 |
