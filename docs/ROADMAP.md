...
|---|---|---|
| **0** | **Post the link** | Ten connectors by name; `/?read&via=group`, `/?eleven&via=group`, `/?door&via=group`; the door's honest count posted weekly. `docs/WEDGE.md` steps 1–4. **Waits on one act — removing `PREVIEW_PASSWORD`** (`docs/DEPLOY.md`; decided 2026-09-12: the quiet launch. The `noindex` came off on 2026-09-13 — it was making the domain read as broken in search — so the same day also needs the Search Console steps in `docs/DEPLOY.md`). Otherwise, nothing.** Every line below waits on it |
