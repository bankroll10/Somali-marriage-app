...
| **15** | The vouch token is eight characters, minted without `onlyIfNew` | 23⁸ ≈ 78 billion, so an even chance of collision at ~330,000 tokens — beyond this product's horizon, and a collision would point a family member's link at the wrong map | **Chose Hard, 2026-09-24** — ten characters since 2026-09-23, and the pointer is now written `onlyIfNew` with a fresh token on collision; report ids the same (`docs/INTEGRITY.md`) |
