...
| **Payments** | None | A hosted payment link for the call after the joint view, once its gate in `docs/MONETIZATION.md` passes; the matchmaker is the founder, paid at the nikah by invoice | Matchmaker payouts — real marketplace infrastructure | Disputes, refunds, tax by country. Never inside an app-store binary (CONTROL) | — |
