...
| `GET /progress` | The ladder: `rungs`, `cohorts` (per arrival month, `arrived` and `followedThrough`), the splits `scenes`, `vias`, `sides` and `sidesByVia`, and `facts` with its `marriedBy` and `followedThroughBy` cross-tabs. Splits are floored, so a cell under five reads `null`. The walk also deletes expired records it passes | Monthly (`docs/RESEARCH.md`). The field-by-field table is in `docs/PRIVACY.md` |
