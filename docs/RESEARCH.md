...
| `why-now` weights (1 / 0.6 / 0.4 / 0.5) | `src/data/intake.ts` | `marriedBy` against the `why-now` answer, once a hundred hold one | "My family and community expect it of me" is weighted lowest, so honesty about pressure reads as thinner intention. If those who name it marry and follow through as often as the rest, the weight is wrong and moves up (`docs/DECISIONS.md` Part 10) |
