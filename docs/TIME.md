...
| **Matching** | **Yes, for what exists.** The eleven and the read are both self-serve engines (`src/lib/beforeYes.ts`, `src/lib/read.ts`) with no human in the loop | Nothing stops. The *founder-run* matchmaker role in `src/data/plus.ts` ("A matchmaker in your corner") is real future labor, but it is priced and scoped for when a city opens — not running yet, so nothing to audit today. **Opening a pool is the founder's decision by design**; since `docs/LIQUIDITY.md` it is a checklist read from `/pool` rather than a feeling — and still one person's hour, which `docs/MACHINE.md` stage 6 names as the failure state: a full door nobody opens |
...
| **Notifications** | **No — and since 2026-09-17 the product no longer promises one.** "The day someone in your pool fits your map, we write to you" was the founder reading the contacts store and mailing by hand, and it was the first thing that stopped in her absence (`docs/BOARD.md`). The sentence is gone from every screen and `tests/promises.test.ts` keeps it out; the door now says nobody is introduced yet and that the screen will say so when that changes (`docs/RISKS.md` R3). The one sweep that was founder labour — lapsed contacts off the door — runs weekly on its own (`netlify/functions/sweep.ts`). A woman who sends her eleven still has to return and check | This is a real product gap, but it does not become worse if the founder disappears — it is already the steady state. Out of scope for this pass, which asks what the founder's *absence* changes |
