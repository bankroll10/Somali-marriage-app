import { PICKUP_END_HOUR, PICKUP_PLACE, PICKUP_PREFERRED_AFTER_HOUR, PICKUP_START_HOUR, PRODUCTS } from '../../shared/config.ts'
...
export const PICKUP_LINE = `${PICKUP_WINDOW} at ${PICKUP_PLACE}`
...
export function describeQty(qty: Qty, style: 'short' | 'long' = 'long'): string {
  const parts = PRODUCTS.filter((p) => qty[p.id] > 0).map((p) =>
    style === 'short' ? `${qty[p.id]} ${p.id === 'banana' ? 'banana' : p.name.toLowerCase()}` : `${qty[p.id]} ${p.name.toLowerCase()}`,
  )
  return parts.join(' · ')
}
