...
export default function Profile({
  ...
  async function askFamily() {
    setAsking('working')
    // A vouch attaches to a kept map; keep it now if she hasn't.
    ...
    const result = await shareOrCopy(
      {
        text: 'Salaam — I’m using Niyyah, a marriage platform built for us. Before anyone is introduced, they ask a family member to confirm I am who I say and that I’m seeking marriage. Would you? It takes a minute, and there’s no account.',
        url: vouchLink(code, SITE_URL),
      },
      'vouch_asked',
    )
            ...
            {vouch ? (
              <div className="mt-4 flex items-center gap-2.5 rounded-2xl border border-forest/25 bg-forest/[0.06] px-4 py-3">
                ...
                <span className="text-[0.92rem] text-ink">
                  ...
                  <span className="text-muted"> · {relationshipLabel(vouch.relationship)}, {vouch.firstName}</span>
                </span>
              </div>
            ) : (
              <button
