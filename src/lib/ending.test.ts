...
import { buildEnding, endingHeadline, marriedShare, type EndingInput } from './ending'
...
describe('the one thing only a married person can send', () => {
  it('says what kind of link it is, and nothing about who sent it', () => {
    const share = marriedShare('Ask about money home before anyone books a hall.')
    expect(share.url).toMatch(/\/tools\/before-you-say-yes\?via=married$/)
    expect(share.url).not.toMatch(/code|map=|ref|name/)
    expect(share.text).not.toMatch(/Hodan|ACDEFG/)
    expect(share.text).toContain('Ask about money home before anyone books a hall.')
    expect(marriedShare().text).not.toMatch(/\n\n/)
  })
